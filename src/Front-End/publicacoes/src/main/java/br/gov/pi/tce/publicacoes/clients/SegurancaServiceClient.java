...
public class SegurancaServiceClient {
	...
	private Invocation.Builder chamadaAPI(Long id) {
		...
		if (id == null) {
			this.webTarget = this.client
					.target(propriedades.getValorString("URI_API") + propriedades.getValorString("URI_SEGURANCA"));
		} else {
			this.webTarget = this.client
					.target(propriedades.getValorString("URI_API") + propriedades.getValorString("URI_SEGURANCA"))
	...
	public RespostaToken pegarToken(RequisicaoToken requisicaoToken) throws Exception {
		Form form = new Form();
		form.param("client", requisicaoToken.getClient()).param("username", requisicaoToken.getUsername())
				.param("password", requisicaoToken.getPassword()).param("grant_type", requisicaoToken.getGrant_type());
