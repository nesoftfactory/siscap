...
public class PublicacaoController extends BeanController {
	...
	public void getDiariosDOM(Long idFonte, Date dataInicial, Date dataFinal, String token) {
		...
		// propriedades.getValorString("EMAIL_FROM"),
		// propriedades.getValorString("EMAIL_SUBJECT"),
		// propriedades.getValorString("EMAIL_CONTENT"));
	...
	private void salvarPublicacaoInexistente(List<LocalDate> diasUteisList, Fonte fonte, String token) {
		for (LocalDate localDate : diasUteisList) {
			...
			if (!isFeriado) {
				Propriedades propriedades = Propriedades.getInstance();
				LOGGER.info(propriedades.getValorString("EMAIL_CONTENT") + fonte.getNome()
						+ propriedades.getValorString("EMAIL_CONTENT_2") + convertDateToString(date) + ".");
				salvarPublicacao(fonte, "", convertDateToString(date), "", Boolean.FALSE, Boolean.FALSE,
						propriedades.getValorString("MENSAGEM_ERRO_DIARIO_NAO_ENCONTRADO"), null, null, "",
						propriedades.getValorString("MENSAGEM_DIARIO_INDISPONIVEL"), token);
			}
		}
	}
	 ...
	 * @return
	 */
	private Boolean getPaginasDiariosDOM(Fonte fonte, String pageDom, List<String> arquivoList, Date dataInicial,
			Date dataFinal, List<LocalDate> diasUteisList, String token) {
		...
		try {
			...
			if (erroUrlFonte) {
				...
				isFinalPaginacao = lerPaginaDiarioDOM(fonte, pageDom, arquivoList, htmlCompleto, isFinalPaginacao,
						regexForDate, regexForPDF, dataInicial, dataFinal, diasUteisList, token);
			}
	 ...
	 * @throws IOException
	 */
	private Boolean lerPaginaDiarioDOM(Fonte fonte, String pageDom, List<String> arquivoList, String htmlCompleto,
			Boolean isFinalPaginacao, String regexForDate, String regexForPDF, Date dataInicial, Date dataFinal,
			List<LocalDate> diasUteisList, String token) throws MalformedURLException, IOException {
			...
			if ((!isNull(regexForDate)) && (!isNull(regexForPDF))) {
				...
				while (matcher.find()) {
					if (!isNull(date)) {
						...
						String arquivoStr = matcher.group();
						if (propriedades.getValorLong("ID_FONTE_DIARIO_OFICIAL_PARNAIBA").equals(fonte.getId())
								|| propriedades.getValorLong("ID_FONTE_DIARIO_OFICIAL_DOS_MUNICIPIOS")
										.equals(fonte.getId())) {

							if (dataInicial.compareTo(date) <= 0 && dataFinal.compareTo(date) >= 0) {
								if (propriedades.getValorLong("ID_FONTE_DIARIO_OFICIAL_PARNAIBA")
										.equals(fonte.getId())) {
									...
									salvarPublicacao(fonte,
											propriedades.getValorString("URL_DOWNLOAD_DOM_PARNAIBA") + arquivoStr,
									...
									salvarPublicacao(fonte,
											propriedades.getValorString("URL_DOWNLOAD_DIARIO_OFICIAL_DOS_MUNICIPIOS")
													+ ano + mes + "/" + arquivoStr,
											convertDateToString(date), arquivoStr, Boolean.TRUE, Boolean.FALSE,
							...
							date = null;
						} else if (propriedades.getValorLong("ID_FONTE_DIARIO_OFICIAL_TERESINA")
								.equals(fonte.getId())) {
							...
							if (!arquivoList.contains(arquivoStr)) {
								...
								if (dataInicial.compareTo(date) <= 0 && dataFinal.compareTo(date) >= 0) {
									...
									do {
										...
										if (matcherPositive.find()) {
											...
											arquivoAnexo = new Arquivo(arquivoAnexoStr, Long.valueOf(10), "tipo",
													propriedades.getValorString("URL_DOWNLOAD_DOM_TERESINA")
															+ arquivoAnexoStr,
													"conteudo".getBytes());
									...
									salvarPublicacao(fonte,
											propriedades.getValorString("URL_DOWNLOAD_DOM_TERESINA") + arquivoStr,
											convertDateToString(date), arquivoStr, Boolean.TRUE,
											Boolean.valueOf(publicacaoAnexo != null), "Sucesso", publicacaoAnexo,
											arquivoAnexo, codigo, publicacaoName, token);
									LocalDate localDate = asLocalDate(date);
									if (diasUteisList.contains(localDate)) {
										diasUteisList.remove(localDate);
									}
	...
	private void salvarPublicacao(Fonte fonte, String linkArquivoPublicacao, String dataPublicacao,
		...
		Arquivo arquivo = new Arquivo(nomeArquivoPublicacao, Long.valueOf(10), "tipo", linkArquivoPublicacao,
				"conteudo".getBytes());
		Publicacao publicacao = new Publicacao(fonte, publicacaoName, dataPublicacao, codigo, arquivo.getId(),
		...
		if (publicacaoConsultada == null) {
			Publicacao publicacaoIndisponivel = new Publicacao(fonte,
					propriedades.getValorString("MENSAGEM_DIARIO_INDISPONIVEL"), dataPublicacao, codigo,
			...
			if (publicacaoRetorno.getQuantidadeTentativas() > 0
					&& (propriedades.getValorInt("QUANTIDADE_VEZES_NOTIFICAR") == 0
							|| (publicacaoRetorno.getQuantidadeTentativas()
									% propriedades.getValorLong("QUANTIDADE_VEZES_NOTIFICAR")) == 0)) {
	...
	private Publicacao consultarPublicacaoPorFonteDataNomeArquivo(Publicacao publicacao) {
		...
		List<Publicacao> publicacaolist = publicacaoServiceClient.consultarPublicacaoPorFonteDataNome(
				publicacao.getFonte().getId(), asLocalDate(convertStringToDate(publicacao.getData())),
				publicacao.getNome());
	...
	/**
	 * Método resposável por chamar a API para consultar Notificação Config por
	 * Tipo, e Ativo.
	 * 
	 * @param tipo
	 * @param ativo
	...
	private List<NotificacaoConfig> consultarNotificacaoConfigPorTipoAtivo(NotificacaoTipo tipo, Boolean ativo) {
		List<NotificacaoConfig> notificacaoConfigList = publicacaoServiceClient
				.consultarNotificacaoConfigPorTipoAtivo(tipo, ativo);
		return notificacaoConfigList;
	}
				...
				buscaPDF: while ((linhaHTML = fonteHTML.readLine()) != null) {
					...
					while (matcher.find()) {
						if (!isNull(date)) {
							...
							// salvarPublicacao(fonte,
							// propriedades.getValorString("URL_DOWNLOAD_DIARIO_OFICIAL_PIAUI") + ano + mes
							// + "/"+ matcher.group(), convertDateToString(date), matcher.group(),
							// Boolean.TRUE, Boolean.FALSE, "Sucesso", null, null, "", matcher.group());
							salvarPublicacao(fonte,
									propriedades.getValorString("URL_DOWNLOAD_DIARIO_OFICIAL_PIAUI") + matcher.group(),
									convertDateToString(date), localPublicacao[1], Boolean.TRUE, Boolean.FALSE,
									"Sucesso", null, null, "", localPublicacao[1], token);
	...
	/**
	 * Método responsável por retornar uma lista de LocalDate dos dias uteis
	 * existentes no intervalo das datas do parâmetro.
	 * 
	 * @param dataInicial
	 * @param dataFinal
	 * @return
	...
	/**
	 * Método responsável por retornar uma lista de string com os meses e anos
	 * concatenados existentes no intervalo das datas do parâmetro.
	 * 
	 * @param dataInicial
	 * @param dataFinal
	 * @return
